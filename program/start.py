...
async def get_uptime(client: Client, message: Message):
    ...
    await message.reply_text(
        "🤖 bot status:\n"
        f"• **‹ مـده الـتـشـغـیـل ›** `{uptime}`\n"
        f"• **‹ وقـت الـبـداء ›** `{START_TIME_ISO}`"
    )
    ...
    for member in m.new_chat_members:
        if member.id == bot_id:
            return await m.reply(
              "n/n/*🌝 شكرا لاضافتي الي مجموعتك*"
              "n/n/كل ما عليك عزيزي عيني مشرف في الجروب"
              "وارسل كلمه ( /userbotjoin ) لدعوه المساعد",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
